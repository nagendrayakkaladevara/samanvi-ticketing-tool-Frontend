...
function buildTreeFromItems(items: Permission[]): PermissionTreeGroup[] {
...
function normalizeTree(raw: unknown, items: Permission[]): PermissionTreeGroup[] {
  const payload = extractPayload(raw)
  ...
  return tree.length > 0 ? tree : buildTreeFromItems(items)
}
